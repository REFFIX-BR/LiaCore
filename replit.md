...
**Assistants Page Filter System (Oct 27, 2025)**: Fixed infinite loading bug on /assistants page. Root cause: queryFn missing `credentials: "include"` caused silent authentication failures, leaving React Query stuck in isLoading state. Solution: (1) Added `credentials: "include"` to fetch in Assistants.tsx and AgentReports.tsx, (2) Structured queryKey with serializable primitives ['/api/assistants/metrics', periodFilter, startDate || null, endDate || null], (3) Added retry: false for fast failure detection, (4) Proper enabled flag for custom period validation. All other pages (Contacts.tsx, Monitor.tsx, Groups.tsx) already had correct credentials configuration. Page now loads successfully with KPIs (Total Conversations, Resolutions, Transfers, Success Rate) and all period filters functional.
