...
- **Barcode Formatting UX Fix**: (1) Enhanced Financial assistant to send barcode in TWO versions: formatted line digitável (36490.00050 00003.305901...) AND continuous version without separators (36490000500000330590...) to prevent customer confusion with isolated check digit. (2) Backend handler uses `.replace(/\D/g, '')` regex to strip ALL non-numeric characters (spaces, dots, hyphens). (3) Prompt includes MANDATORY rule requiring both versions with explanatory note.
- **Agent Reports Critical Fix**: Corrected `getAgentReports` calculation that was severely undercounting resolved conversations. Now counts conversations with `status='resolved'` OR `status='awaiting_nps'` OR `resolvedBy IS NOT NULL`. Example: Daniele Cunha Fontana had 21 assigned conversations, old method showed 0 resolved (only counted status='resolved'), corrected method shows 6 resolved (includes conversations in 'queued'/'active' status that were actually resolved by agents). This fix ensures accurate performance metrics and success rates for all agents in the dashboard.
- **Sales Leads Management Tab**: New dedicated "Leads" tab in Sales dashboard (Gestão Comercial) for improved pipeline visualization. (1) Displays complete pre-installation pipeline (Prospecção, Aguardando Análise, Aprovado, Cancelado) excluding only installed/scheduled sales. (2) KPIs include total leads, awaiting analysis count, approved count, and conversion rate (approved/total). (3) Dual-layer filters: status (Prospecção, Aguardando Análise, Aprovado, Cancelado) and origin (WhatsApp, Site, Manual). (4) Detailed lead view with customer info, contact details, plan of interest, location, and conversation link for WhatsApp-sourced leads. (5) Status update dialog with observations field for tracking lead progression through sales funnel.
- **Complete Lead Capture System (2025-10-22)**: Comprehensive system for capturing and managing incomplete sales prospects via multiple mechanisms: (1) **Manual Function `registrar_lead_prospeccao()`** - AI Commercial assistant can now save "Prospecção" status leads when customers show interest but don't complete registration (requires name + phone, accepts optional email/city/plan). (2) **Automatic Backup System** - Auto-closure worker detects abandoned commercial conversations (3+ client messages, no sale registered) and automatically saves "Prospecção" leads to prevent data loss. (3) **Updated Leads Tab** - Now includes "Lead Sem Cobertura" status (customers interested but outside coverage area) with dedicated KPI card and gray badge color. (4) **Enhanced Commercial Prompt** - Detailed instructions with 2 practical examples showing when to use each of 3 lead functions: `registrar_lead_sem_cobertura()` (no coverage), `registrar_lead_prospeccao()` (interest without completion), `enviar_cadastro_venda()` (complete registration). System ensures zero lead loss by combining manual assistant actions with automatic detection of abandoned conversations.
- **Assistant Routing Context Bug Fix (2025-10-22)**: Corrected critical bug in internal AI routing system where assistants would paraphrase customer requests instead of responding to them. When one AI assistant transfers conversation to another (e.g., Presentation → Commercial), the context injection prompt now explicitly instructs: "You must RESPOND to the customer (do not paraphrase or repeat what they said)". Fixed in 3 locations: (1) Internal routing between specialized assistants (`rotear_para_assistente`), (2) Receptionist routing to specialized departments (first occurrence), (3) Receptionist routing (Evolution API flow). Example: Customer says "I want internet plans" → Old behavior: New assistant says "Hello! I would like to know about internet plans..." → New behavior: "Hello! I'm the commercial assistant and I'll help you with our internet plans. What are your needs?"
