...
**Regional Massive Failure System**: Automated detection and notification system for service outages affecting specific neighborhoods. Includes visual region selector for creating failures, automatic client notification via WhatsApp when affected by outages, and intelligent region matching that normalizes city/neighborhood names (handles extra spaces and accents) from CRM API to database records. Serves 7 cities with 145 neighborhoods total.
