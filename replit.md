...
- **Message Deduplication System (Dec 2024)**: Three-layer protection against duplicate message saves caused by Evolution API sending webhooks twice (~100ms apart):
  1. **Application Pre-check**: Before saving, routes.ts checks if `whatsappMessageId` already exists in database and returns early if duplicate detected
  2. **Database UNIQUE Index**: Partial UNIQUE index `messages_whatsapp_message_id_unique` on `whatsapp_message_id` column (excludes NULL values for legacy messages) - defined in schema.ts for all environments
  3. **Conflict Handler**: storage.ts `createMessage` catches PostgreSQL error 23505 (unique violation) and returns the existing message instead of failing
  - This handles race conditions where both webhook requests pass application checks before either inserts
