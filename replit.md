...
- **Manual Transfer Department Tagging Fix** (Dec 15, 2025): CORRIGIDO - Transferência manual para "Financeiro" era taggeada como "Suporte". Causa: Endpoint `/api/supervisor/transfer` atualizava `department` mas NÃO atualizava `assistantType`. Solução: Adicionado mapeamento `departmentToAssistant` para atualizar AMBOS os campos. Agora: Financeiro → `department: 'financial'` + `assistantType: 'financeiro'`. Local: `server/routes.ts` linhas 5104-5125.
