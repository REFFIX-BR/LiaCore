...
**Department-Based Access Control**: Agents can be assigned to multiple departments (general, commercial, financial, support, cancellation). Conversations are automatically tagged with departments based on the AI assistant that handled them. Agents only see conversations from their assigned departments in transferred/assigned queues. Supervisors and admins see all conversations. Backward-compatible with legacy data (conversations without departments are visible to all agents during migration).
