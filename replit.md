...
**Massive Failure Detection Fix** (Oct 27, 2025): ✅ FIXED - Resolved two critical bugs preventing massive failure detection from working:
1. **JSON Parse Error**: Fixed `fetchClientInstallationPoints` function in `massive-failure-handler.ts` which was throwing "Unexpected end of JSON input" when CRM API returned empty responses. Added proper text validation and try-catch block before JSON parsing (lines 47-59).
2. **Incorrect JSON Format**: Fixed `affected_regions` JSON structure in `massive_failures` table. The correct format is `{"type": "custom", "custom": [{"city": "CITY_NAME", "neighborhoods": ["NEIGHBORHOOD1", "NEIGHBORHOOD2"]}]}` for custom regions, or `{"type": "predefined", "regionIds": ["uuid1", "uuid2"]}` for predefined regions. The `checkActiveFailureForRegion` function in `server/storage.ts` (line 3882) expects this exact structure to match client locations against active failures. Files: `server/lib/massive-failure-handler.ts`, `server/storage.ts`.
