...
**Assistants Page Filter System (Oct 27, 2025)**: Fixed infinite loading bug when changing period filters. Root cause: React Query cache invalidation issue - queryKey was using full URL string causing new queries instead of cache updates. Solution: Structured queryKey array with explicit parameters (periodFilter, startDate, endDate), explicit queryFn with typed return, and proper boolean conversion for enabled flag. Custom date picker now properly disables query until both dates selected.
