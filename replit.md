...
**Prompt Management System**: Features an AI-powered prompt editor with version control, draft workflows, semantic versioning, GPT-4o AI analysis for recommendations, real-time token counting, and automatic OpenAI Assistants API synchronization.
...
**LIA VOICE - Hybrid Debt Collection Module**: Production-ready implementation supporting proactive voice calls (Twilio + OpenAI Realtime API) and automated WhatsApp messaging (Evolution API + IA Financeiro). Features include 6 BullMQ workers, a hybrid routing system based on `contactMethod`, a WhatsApp Collection Worker, a centralized WhatsApp module, UI contact method selector, Twilio integration with secure webhooks, OpenAI Realtime API via WebSocket, session security, ANATEL compliance, graceful degradation, and campaign management with statistics auto-recalculation.
**Messaging Control Center**: Professional control panel for flexible contact method management with global and per-campaign configuration, including global settings (voice/WhatsApp toggles, default method, fallback order), per-campaign overrides (allowedMethods, fallbackOrder in VoiceCampaigns drawer), backend validation (prevents activation if configured methods globally disabled, validates BEFORE database update), and method-specific statistics API (GET /api/voice/stats returns separated metrics by contactMethod with backward compatibility).
**Campaign Conversation Tracking**: Complete integration between voice campaigns and the supervisor dashboard. Conversations table includes `conversationSource` field (values: 'inbound', 'voice_campaign', 'whatsapp_campaign') and `voiceCampaignTargetId` for bidirectional linking with campaign targets. WhatsApp collection worker creates conversations before sending messages, registering initial collection messages and linking to campaign targets. When reusing existing conversations, the worker updates them to mark as 'whatsapp_campaign' and link to targets. Supervisor dashboard includes hierarchical "Cobranças" menu with 4 filter options: 📱 Todas, 📥 Entrada, 💬 Campanha WhatsApp, 📞 Campanha Voz. The menu uses a dropdown interface with visual check marks indicating the active filter. This enables supervisors to monitor all debt collection interactions through the unified dashboard with an improved UX.
