...
export interface IStorage {
  ...
  getSatisfactionFeedbackWithConversations(period?: 'daily' | 'weekly' | 'monthly'): Promise<Array<SatisfactionFeedback & { conversation?: Conversation; agentName?: string }>>;
...
export class DbStorage implements IStorage {
  ...
  async updateConversation(id: string, updates: Partial<Conversation>): Promise<Conversation | undefined> {
    // NOTE: admin_bulk_close protection REMOVED
    // Individual customer messages should always be processed normally
    // The flag was causing legitimate customer messages to be blocked
    // If mass reopen detection is needed in the future, it should be implemented
    // at the webhook level with proper rate limiting, not by blocking status updates
  ...
  async getMessagesPaginated(conversationId: string, options: { limit?: number; before?: string }): Promise<{ messages: Message[]; hasMore: boolean }> {
    ...
    let query = db.select({
      ...
      audioBase64: schema.messages.audioBase64,
      videoUrl: schema.messages.videoUrl,
      videoName: schema.messages.videoName,
      videoMimetype: schema.messages.videoMimetype,
      locationLatitude: schema.messages.locationLatitude,
      locationLongitude: schema.messages.locationLongitude,
      whatsappMessageId: schema.messages.whatsappMessageId,
      remoteJid: schema.messages.remoteJid,
      whatsappStatus: schema.messages.whatsappStatus,
      whatsappStatusUpdatedAt: schema.messages.whatsappStatusUpdatedAt,
      whatsappRetryCount: schema.messages.whatsappRetryCount,
      whatsappLastRetryAt: schema.messages.whatsappLastRetryAt,
      whatsappTemplateMetadata: schema.messages.whatsappTemplateMetadata,
  ...
  async getSatisfactionFeedbackWithConversations(period?: 'daily' | 'weekly' | 'monthly'): Promise<Array<SatisfactionFeedback & { conversation?: Conversation; agentName?: string }>> {
    const now = new Date();
    let startDate: Date;
    ...
    return feedbacks.map(feedback => {
      const conversation = conversationMap.get(feedback.conversationId);
      const agentId = conversation?.firstResolvedBy || conversation?.resolvedBy;
      return {
        ...feedback,
        conversation,
        agentName: agentId ? agentMap.get(agentId) : undefined
      };
    });
  }
  ...
  async getAgentReports(params: {
    ...
    // ✅ FIX: Usar a DATA DE RESOLUÇÃO para agrupar (mesmo período que as resolvidas)
    // "Atendidas" = conversas atribuídas (assignedTo) que foram resolvidas no período
  ...
  async calculateGamificationScores(period: string): Promise<void> {
    ...
    for (const agent of agents) {
      ...
      // 2. Tempo de Atendimento: da transferência até resolução
      ...
      for (const conv of conversations) {
        // 1. Tempo de Primeira Resposta: primeira mensagem humana (sendBy='supervisor') após transferência
        if (conv.transferredAt) {
          ...
          if (firstHumanMessage.length > 0 && firstHumanMessage[0].timestamp) {
          ...
          // 2. Tempo de Atendimento: da transferência até resolução
          if (conv.resolvedAt) {
            const serviceTime = conv.resolvedAt.getTime() - conv.transferredAt.getTime();
            if (serviceTime > 0) {
