...
// Supported instances: Principal, Leads, Cobranca
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // Para desenvolvimento local, sempre usar secure: false
      const cookieOptions: any = {
        httpOnly: true,
        secure: false, // Sempre false para HTTP local
        sameSite: "lax",
        maxAge: 7 * 24 * 60 * 60 * 1000, // 7 days
        path: "/", // Garantir que o cookie seja enviado para todas as rotas
      };
      
      // Não definir domain para permitir que funcione em qualquer subdomínio/localhost
      // Se definir domain, pode causar problemas em localhost
      
      res.cookie("auth_token", token, cookieOptions);
      
      console.log(`🍪 [Auth] Cookie definido para ${user.username} (secure: ${cookieOptions.secure}, path: ${cookieOptions.path}, maxAge: ${cookieOptions.maxAge}ms)`);
    ...
    try {
      ...
      if (conversation.status === 'resolved') {
        ...
        const updateData: any = {
          ...
          evolutionInstance: 'Principal', // Reaberturas usam instância Principal
        };
        
        console.log(`📱 [Reopen] Usando instância Principal para reabertura de conversa`);
        ...
        // CRITICAL FIX: Se conversa está atribuída a um colaborador, NÃO resetar transferência
        // A conversa deve permanecer atribuída até o colaborador transferir ou finalizar
        if (conversation.assignedTo) {
          console.log(`👤 [Active] Conversa atribuída a colaborador (${conversation.assignedTo}) - mantendo atribuição`);
          // Apenas atualizar lastMessageTime, não resetar transferência
          await storage.updateConversation(conversation.id, {
            lastMessageTime: new Date(),
          });
        } else {
          // Conversa transferida mas NÃO atribuída - resetar para IA responder
          console.log(`🤖 [Active Reset] Conversa transferida mas não atribuída - resetando para IA: ${chatId}`);
      ...
      if (event === "messages.upsert") {
        ...
        if (isGroup) {
          ...
          if (!group) {
            ...
            // Grupos usam instância Principal
            console.log(`📱 [Groups] Usando instância Principal para novo grupo`);
            
            group = await storage.createGroup({
              groupId,
              name: groupName,
              avatar: groupAvatar,
              evolutionInstance: 'Principal', // Grupos usam instância Principal
...
Qualquer coisa, estamos à disposição! 😊
        ...
        if (conversation.status === 'resolved') {
          ...
          console.log(`📱 [Evolution Reopen] Usando instância Principal para reabertura`);
          ...
          // A conversa deve permanecer atribuída até o colaborador transferir ou finalizar
          if (conversation.assignedTo) {
            console.log(`👤 [Evolution Active] Conversa atribuída a colaborador (${conversation.assignedTo}) - mantendo atribuição`);
            // Apenas atualizar lastMessageTime, não resetar transferência
            await storage.updateConversation(conversation.id, {
              lastMessageTime: new Date(),
            });
            ...
            // Conversa transferida mas NÃO atribuída - resetar para IA responder
            console.log(`🤖 [Evolution Active Reset] Conversa transferida mas não atribuída - resetando para IA: ${chatId} (${clientName})`);
...
- Se necessário, pedir informações adicionais para melhor ajudar`;
  ...
  app.post("/api/conversations/:id/reopen", authenticate, requireAdminOrSupervisor, async (req, res) => {
    try {
      ...
      console.log(`📱 [Reopen] Reabrindo conversa para fila de atribuição - instância Principal`);
      await storage.updateConversation(id, {
        status: 'queued',
        transferredToHuman: true,
        assignedTo: null,
  ...
  app.post("/api/contacts/reopen", authenticate, async (req, res) => {
    try {
      ...
      if (!conversation) {
        // Reaberturas de contato usam instância Principal
        let evolutionInstance = 'Principal'; // Reaberturas usam instância Principal
        console.log(`📱 [Contacts] Usando instância Principal para reabertura de contato`);
        ...
        if (contact.lastConversationId) {
          try {
            ...
            if (lastConversation) {
              // NÃO reutilizar evolutionInstance - sempre usar Principal para reaberturas
        ...
        // Reaberturas usam instância Principal
        console.log(`📱 [Contacts] Usando instância Principal para reabertura de conversa existente`);
        await storage.updateConversation(conversation.id, {
          status: 'active',
          transferredToHuman: true,
          transferReason: 'Conversa reaberta pelo atendente via painel de Contatos',
        ...
        console.log(`✅ [Contacts] Reactivated existing conversation with evolutionInstance=Principal and moved to Transferidas: ${conversation.id}`);
      }
  ...
  app.post("/api/groups/:id/send", authenticate, async (req, res) => {
    try {
      ...
      // Grupos SEMPRE usam instância Principal
      const instance = 'Principal';
      console.log(`📱 [Groups] Usando instância Principal para envio de mensagem ao grupo`);
