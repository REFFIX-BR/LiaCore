...
// =============================================================================
// PAYMENT PROOF INSTALLATION POINT SELECTION SYSTEM
// =============================================================================

export interface PaymentProofMenuItem {
  numero: number;
  endereco: string;
  bairro: string;
  cidade: string;
...
export interface PaymentProofMenu {
  conversationId: string;
  cpf: string;
  pontos: PaymentProofMenuItem[];
  paymentDetails: {
    valor?: string;
    data?: string;
    tipo?: string; // "PIX", "BOLETO", "TED"
  };
  createdAt: number;
}

/**
 * Sistema efêmero para gerenciar seleção de pontos para comprovantes de pagamento
 * Similar ao sistema de boletos, mas para abertura de tickets
 */
export class PaymentProofSelectionManager {
  private readonly MENU_TTL = 300; // 5 minutos
  private readonly FLAG_TTL = 300; // 5 minutos
  
  async saveMenu(menu: PaymentProofMenu): Promise<void> {
    try {
      const key = `menu:payment:${menu.conversationId}`;
      await redis.set(key, JSON.stringify(menu), { ex: this.MENU_TTL });
      
      await this.setAwaitingSelection(menu.conversationId, true);
      
      console.log(`💾 [Payment Menu] Menu salvo para conversa ${menu.conversationId} (${menu.pontos.length} pontos, TTL: ${this.MENU_TTL}s)`);
    } catch (error) {
      console.error(`❌ [Payment Menu] Erro ao salvar menu:`, error);
      throw error;
    }
  }
  
  async getMenu(conversationId: string): Promise<PaymentProofMenu | null> {
    try {
      const key = `menu:payment:${conversationId}`;
      const data = await redis.get(key);
      
      if (!data) {
        console.log(`💾 [Payment Menu] Menu não encontrado para conversa ${conversationId}`);
        return null;
      }
      
      const menu = typeof data === 'string' ? JSON.parse(data) : data;
      console.log(`💾 [Payment Menu] Menu recuperado para conversa ${conversationId} (${menu.pontos?.length || 0} pontos)`);
      return menu;
    } catch (error) {
      console.error(`❌ [Payment Menu] Erro ao recuperar menu:`, error);
      return null;
    }
  }
  ...
  async deleteMenu(conversationId: string): Promise<void> {
    try {
      ...
      await redis.del(key);
      await this.setAwaitingSelection(conversationId, false);
      console.log(`🗑️ [Payment Menu] Menu removido para conversa ${conversationId}`);
    } catch (error) {
      console.error(`❌ [Payment Menu] Erro ao remover menu:`, error);
    }
  }
  
  async setAwaitingSelection(conversationId: string, awaiting: boolean): Promise<void> {
    try {
      const key = `awaiting:payment:${conversationId}`;
      
      if (awaiting) {
        await redis.set(key, '1', { ex: this.FLAG_TTL });
        console.log(`🚩 [Payment Selection] Flag ATIVA para conversa ${conversationId}`);
      } else {
        await redis.del(key);
        console.log(`🚩 [Payment Selection] Flag REMOVIDA para conversa ${conversationId}`);
      }
    } catch (error) {
      console.error(`❌ [Payment Selection] Erro ao definir flag:`, error);
    }
  }
  ...
  async isAwaitingSelection(conversationId: string): Promise<boolean> {
    try {
      ...
      console.log(`🚩 [Payment Selection] Conversa ${conversationId} aguardando seleção: ${awaiting}`);
      return awaiting;
    } catch (error) {
      console.error(`❌ [Payment Selection] Erro ao verificar flag:`, error);
      return false;
    }
  }
  ...
  mapClientResponseToPointNumber(
    ...
    const directNumber = parseInt(messageLower);
    if (!isNaN(directNumber) && directNumber >= 1 && directNumber <= menu.pontos.length) {
      const pontoIndex = directNumber - 1;
      const ponto = menu.pontos[pontoIndex];
      if (ponto) {
        console.log(`🎯 [Payment Mapping] Cliente escolheu ${directNumber}º item → ponto ${ponto.numero}`);
        return ponto.numero;
      }
    }
    ...
    for (const [ordinal, numero] of Object.entries(ordinaisMap)) {
      if (messageLower.includes(ordinal)) {
        const pontoIndex = numero - 1;
        const ponto = menu.pontos[pontoIndex];
        if (ponto) {
          console.log(`🎯 [Payment Mapping] Cliente escolheu ${numero}º item via ordinal "${ordinal}" → ponto ${ponto.numero}`);
          return ponto.numero;
        }
      }
    }
    
    console.log(`⚠️ [Payment Mapping] Não foi possível mapear "${clientMessage}" para número de ponto`);
    return null;
  }
}

// Instância global
export const paymentProofManager = new PaymentProofSelectionManager();
