...
### C.9 Fluxograma Completo: Cliente → IA → Atendente Humano

#### C.9.1 Visão Geral do Fluxo

```
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│                    FLUXO COMPLETO: MENSAGEM DO CLIENTE ATÉ RESOLUÇÃO                   │
├─────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                         │
│  ┌──────────┐      ┌──────────┐      ┌──────────┐      ┌──────────────────────────┐    │
│  │ CLIENTE  │      │ WHATSAPP │      │ EVOLUTION│      │      LIA CORTEX          │    │
│  │          │─────▶│          │─────▶│   API    │─────▶│      (REPLIT)            │    │
│  │ Mensagem │      │          │      │ Webhook  │      │                          │    │
│  └──────────┘      └──────────┘      └──────────┘      └────────────┬─────────────┘    │
│                                                                      │                  │
...
│  │  │ 2. Busca/Cria conversa no banco                                             │ │  │
│  │  │ 3. Verifica status: WAITING_AI / TRANSFERRED / ASSIGNED                     │ │  │
│  │  └───────────────────────────────────┬─────────────────────────────────────────┘ │  │
│  │                                      │                                           │  │
│  │                    ┌─────────────────┴─────────────────┐                         │  │
│  │                    │                                   │                         │  │
│  │                    ▼                                   ▼                         │  │
│  │         ┌──────────────────┐                ┌──────────────────┐                 │  │
│  │         │  WAITING_AI      │                │ TRANSFERRED ou   │                 │  │
│  │         │  (Atendimento IA)│                │ ASSIGNED         │                 │  │
│  │         └────────┬─────────┘                │ (Atendimento     │                 │  │
│  │                  │                          │  Humano)         │                 │  │
│  │                  ▼                          └────────┬─────────┘                 │  │
│  │  ┌───────────────────────────────┐                   │                           │  │
...
│  │  ┌───────────┐       ┌───────────┐         ┌─────────────────┐                   │  │
│  │  │ 🔵 BLUE   │       │ 🟢 GREEN  │         │  Notifica       │                   │  │
│  │  │  OpenAI   │       │ Vertex AI │         │  Atendente      │                   │  │
│  │  │  GPT-5    │       │ Gemini    │         │  via Dashboard  │                   │  │
│  │  └─────┬─────┘       └─────┬─────┘         └─────────────────┘                   │  │
│  │        │                   │                                                     │  │
│  │        └─────────┬─────────┘                                                     │  │
│  │                  │                                                               │  │
...
│                                                                                         │
│  ┌──────────────────────────────────────────────────────────────────────────────────┐  │
│  │                         ASSISTENTE RECEPCIONISTA (CORTEX)                        │  │
│  │                                                                                  │  │
│  │  1. Analisa mensagem do cliente                                                  │  │
│  │  2. Identifica intenção (venda, financeiro, suporte, cobrança, etc.)             │  │
│  │  3. Roteia para assistente especializado                                         │  │
│  │                                                                                  │  │
│  │        rotear_para_assistente("comercial" | "financeiro" | "suporte" | ...)      │  │
│  │                                                                                  │  │
│  └────────────────────────────────────┬─────────────────────────────────────────────┘  │
│                                       │                                                │
│                                       ▼                                                │
...
│  │                                                                                  │  │
│  │  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐   │  │
│  │  │Comercial│  │Financei-│  │ Suporte │  │Cobrança │  │Ouvidoria│  │Cancela- │   │  │
│  │  │         │  │   ro    │  │ Técnico │  │         │  │         │  │  mento  │   │  │
│  │  └────┬────┘  └────┬────┘  └────┬────┘  └────┬────┘  └────┬────┘  └────┬────┘   │  │
│  │       │            │            │            │            │            │        │  │
│  │       └────────────┴────────────┴────────────┴────────────┴────────────┘        │  │
...
│  │  │  - buscar_cep(cep)                                                      │    │  │
│  │  │  - consultar_plano_cliente(cpf)                                         │    │  │
│  │  │  - verificar_status_ordem_servico(numero)                               │    │  │
│  │  │  - registrar_promessa_pagamento(dados)                                  │    │  │
│  │  │  - buscar_conhecimento_rag(query)                                       │    │  │
│  │  │  - rotear_para_assistente(assistente)                                   │    │  │
│  │  │  - transferir_para_humano(departamento, motivo, contexto)   ◄── CHAVE   │    │  │
│  │  │                                                                         │    │  │
│  │  └─────────────────────────────────────────────────────────────────────────┘    │  │
│  │                                       │                                          │  │
│  └───────────────────────────────────────┼──────────────────────────────────────────┘  │
...
│                          ▼                               ▼                             │
│              ┌─────────────────────┐         ┌─────────────────────┐                   │
│              │  IA RESOLVE         │         │  TRANSFERE PARA     │                   │
│              │                     │         │  HUMANO             │                   │
│              │  Envia resposta     │         │                     │                   │
│              │  ao cliente         │         │  transferir_para_   │                   │
│              │                     │         │  humano()           │                   │
│              └──────────┬──────────┘         └──────────┬──────────┘                   │
│                         │                               │                              │
│                         ▼                               ▼                              │
│              ┌─────────────────────┐         ┌─────────────────────────────────────┐   │
│              │  WhatsApp           │         │  ATUALIZA BANCO DE DADOS            │   │
│              │  Evolution API      │         │                                     │   │
│              │  → Cliente          │         │  conversation.status = 'transferred'│   │
│              │                     │         │  conversation.assignedTo = null     │   │
│              │  ✅ RESOLVIDO       │         │  conversation.department = "X"      │   │
│              │     PELA IA         │         │  conversation.transferContext = {}  │   │
│              └─────────────────────┘         └──────────────┬──────────────────────┘   │
│                                                             │                          │
└─────────────────────────────────────────────────────────────┼──────────────────────────┘
                                                              │
...
│  │  │                                                                         │    │  │
│  │  │  Cliente: João Silva (5524999887766)                                    │    │  │
│  │  │  Departamento: Suporte Técnico                                          │    │  │
│  │  │  Motivo: "Cliente precisa de técnico - OS deve ser aberta"              │    │  │
│  │  │  Urgência: ALTA (24h+ sem internet)                                     │    │  │
│  │  │  Tempo de espera: 0m                                                    │    │  │
│  │  │                                                                         │    │  │
│  │  │  [Assumir Conversa]  [Ver Histórico]  [Atribuir para Agente]            │    │  │
│  │  └─────────────────────────────────────────────────────────────────────────┘    │  │
│  │                                                                                  │  │
│  └────────────────────────────────────┬─────────────────────────────────────────────┘  │
│                                       │                                                │
...
│  │                         ATENDENTE ASSUME                                         │  │
│  │                                                                                  │  │
│  │  1. Clica em "Assumir Conversa"                                                  │  │
│  │  2. conversation.status = 'assigned'                                             │  │
│  │  3. conversation.assignedTo = agente.id                                          │  │
│  │                                                                                  │  │
│  │  ┌─────────────────────────────────────────────────────────────────────────┐    │  │
│  │  │  💬 INTERFACE DO AGENTE                                                 │    │  │
│  │  │                                                                         │    │  │
│  │  │  [Histórico completo da conversa com IA]                                │    │  │
...
│  │  │  └─────────────────────────────────────────────────────────────────┘   │    │  │
│  │  │                                                                         │    │  │
│  │  │  [Usar Sugestão IA]  [Devolver para IA]  [Resolver]                     │    │  │
│  │  └─────────────────────────────────────────────────────────────────────────┘    │  │
│  │                                                                                  │  │
│  └────────────────────────────────────┬─────────────────────────────────────────────┘  │
│                                       │                                                │
...
│                          ▼                         ▼                                   │
│              ┌─────────────────────┐   ┌─────────────────────┐                         │
│              │  ATENDENTE RESPONDE │   │  ATENDENTE RESOLVE  │                         │
│              │                     │   │                     │                         │
│              │  → WhatsApp         │   │  status = 'resolved'│                         │
│              │  → Cliente          │   │  resolvedBy = agent │                         │
│              │                     │   │  resolvedAt = now() │                         │
│              └─────────────────────┘   └─────────────────────┘                         │
│                                                                                         │
└─────────────────────────────────────────────────────────────────────────────────────────┘
...
#### C.9.2 Detalhamento: Quando a IA Transfere para Humano
...
│                                                                             │
│  COMERCIAL                                                                  │
│  ├── Cliente quer mudança de endereço (agendamento necessário)              │
│  ├── Cliente quer falar com vendedor específico                             │
│  ├── Negociação especial (desconto além do permitido)                       │
│  └── Cliente frustrado/irritado                                             │
│                                                                             │
│  FINANCEIRO                                                                 │
│  ├── Contestação de cobrança                                                │
│  ├── Divergência de valores                                                 │
│  ├── Débito muito antigo (> 90 dias)                                        │
│  ├── Já fez 1 desbloqueio nos últimos 7 dias                                │
│  └── Cliente revoltado com cobrança                                         │
│                                                                             │
│  SUPORTE TÉCNICO                                                            │
│  ├── Precisa de técnico presencial                                          │
│  ├── Problema persiste após troubleshooting                                 │
│  ├── Sem internet há mais de 24h (URGENTE)                                  │
│  ├── Cliente já ligou 3+ vezes                                              │
│  └── Problema em equipamento específico                                     │
│                                                                             │
│  COBRANÇA                                                                   │
│  ├── Já fez promessa nos últimos 15 dias                                    │
│  ├── Alegação de prescrição                                                 │
│  ├── Divergência de valores                                                 │
│  ├── Cliente revoltado                                                      │
│  └── Fora do horário ANATEL (8h-18h)                                        │
│                                                                             │
│  OUVIDORIA                                                                  │
│  ├── Reclamação formal                                                      │
│  ├── Problema não resolvido anteriormente                                   │
│  └── Solicitação de protocolo                                               │
│                                                                             │
│  CANCELAMENTO                                                               │
│  ├── Sempre transfere (retenção precisa humano)                             │
│  └── Cliente decidido a cancelar                                            │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### C.9.3 Código da Função transferir_para_humano

```typescript
// Função chamada pela IA quando precisa transferir para humano
async function transferir_para_humano(params: {
  departamento: string;      // "Suporte" | "Financeiro" | "Comercial" | etc.
  motivo: string;            // Motivo da transferência
  contexto: string;          // Contexto detalhado para o atendente
  urgencia?: 'baixa' | 'media' | 'alta' | 'urgente';
}): Promise<TransferResult> {
  
  const { departamento, motivo, contexto, urgencia = 'media' } = params;
  
  // 1. Atualiza status da conversa no banco
  await db.update(conversations)
    .set({
      status: 'transferred',
      assignedTo: null,           // Ainda não atribuído
      department: departamento,
      transferContext: JSON.stringify({
        motivo,
        contexto,
        urgencia,
        transferidoEm: new Date().toISOString(),
        assistenteOrigem: currentAssistant
      }),
      waitingSince: new Date()    // Para calcular tempo de espera
    })
    .where(eq(conversations.id, conversationId));
  
  // 2. Envia mensagem ao cliente informando
  await enviarMensagemWhatsApp(clientPhone, 
    `Um momento, estou transferindo você para um atendente do setor ${departamento}. ` +
    `Aguarde, em breve você será atendido.`
  );
  
  // 3. Notifica supervisores/atendentes via WebSocket
  ...
  await registrarAtividade({
    ...
    details: { departamento, motivo, contexto, urgencia }
  });
  
  return {
    success: true,
    message: `Transferido para ${departamento}`,
    waitPosition: await calcularPosicaoFila(departamento)
  };
}
```

#### C.9.4 Fluxo no Modelo Blue-Green (ADK)

No modelo ADK, a transferência para humano funciona igual, mas é implementada como uma Tool Python:

```python
# Google ADK - Tool de transferência para humano
from google.adk.tools import FunctionTool
import httpx
...
async def transferir_para_humano(
    ...
    Use quando:
    - Cliente precisa de atendimento presencial
    - Problema não pode ser resolvido via chat
    - Cliente solicita falar com humano
    - Caso requer autorização especial
    
    Args:
        departamento: "Suporte" | "Financeiro" | "Comercial" | "Cobrança" | "Ouvidoria"
        motivo: Breve descrição do motivo da transferência
        contexto: Contexto detalhado para o atendente (histórico, tentativas, etc.)
        urgencia: "baixa" | "media" | "alta" | "urgente"
    ...
    # Chama API do LIA CORTEX (Replit) para processar transferência
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "https://lia-cortex.trtelecom.repl.co/api/transfer",
            json={
...
# Registrar como tool do agente
transfer_tool = FunctionTool(transferir_para_humano)
```

#### C.9.5 Fluxo Resumido (Diagrama Simples)

```
     CLIENTE                    SISTEMA                         ATENDENTE
        │                          │                                │
        │  "Oi, preciso de ajuda"  │                                │
        ├─────────────────────────▶│                                │
        │                          │                                │
